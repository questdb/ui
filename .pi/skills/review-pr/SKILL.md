---
name: review-pr
description: Review a code change (PR, commit, or staged/unstaged diff) against QuestDB Web Console coding standards. Use when asked to review a PR or diff, given a PR number or URL, a commit hash, or `staged`/`unstaged`, optionally with a review level 0..3. Performs an adversarial, blocking, mission-critical frontend code review.
allowed-tools: bash read subagent
metadata:
  argument-hint: "[PR number, PR URL, commit hash, unstaged changes, staged changes] [--level=0..3]"
---

# Review a QuestDB Web Console change

Review the change identified by the arguments you were invoked with. When this
skill is run as `/skill:review-pr <args>`, the `<args>` are appended below as a
`User:` message — treat that text as `$ARGUMENTS` everywhere this document
refers to it. If no review target is present in the arguments, ask the user what
to review (PR number/URL, commit hash, or `staged`/`unstaged`) before doing
anything else.
...
- **Read the full context of changed files** when the diff alone is ambiguous. Use `read` and `bash` (`rg`/`grep`/`find`) to inspect the surrounding code, callers, event handlers, and related tests.
...
| Level           | What runs                                                                                                                                                                                                                                                                                          |
|-----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **0**           | Steps 1, 2, 4. Skip Step 2.5. Skip Step 3a — no agent spawn; review the diff inline in the main loop, using `read`/`bash` on demand to resolve ambiguities. Step 3b runs `yarn typecheck` and `yarn lint` only. Verify each finding inline as you write it. Single-pass review of the diff itself.        |
...
Step 3a (and the verification pass in Step 3c) call for "agents" launched "in
parallel". In pi, launch them with the `subagent(...)` tool using fresh-context
`reviewer` agents — one task per agent role. Each task must be self-contained
because the child does not inherit this conversation. Give every review task:

1. the diff under review (paste it, or have the child re-run the `gh`/`git` command from Step 1),
...
3. the specific role instructions for that agent (Agent 1..13 below),
...
subagent({
  tasks: [
    { agent: "reviewer", task: "Agent 1 — Query execution & data integrity. <diff + change surface map + Agent 1 instructions>. Review only; do not edit." },
    { agent: "reviewer", task: "Agent 2 — React correctness & hooks. <...>. Review only; do not edit." },
    { agent: "reviewer", task: "Agent 3 — Async, timers & cancellation. <...>. Review only; do not edit." },
    { agent: "reviewer", task: "Agent 9 — Test review & coverage. <...>. Review only; do not edit." }
  ],
  context: "fresh"
})
```

For the verification pass (Step 3c), launch verification `reviewer` agents the
same way — one per finding at level 3, a single batched verification agent at
level 2, and inline (no spawn) at levels 0-1. The fresh-context adversarial
agent (Agent 13, level 3 only) must NOT receive the change surface map or the
agent checklists; give it only the diff and the changed-file names, per its
instructions. The parent session owns the Step 3b quality gate (`yarn`
typecheck/lint/build/test:unit), synthesis, deduplication, and the final
report — children only return findings.
...
Before launching review agents, produce a structured change surface map. This step is mandatory at levels 2 and 3 (level 1 runs 2.5a only) and must use `bash` (`rg`/`grep`/`find`) plus `read` — do not reason about consumers from memory. The output of this step is required input for every agent in Step 3a.
...
For every changed symbol that is exported (component, hook, util, client method, context value, action, selector, Dexie accessor, type), run a repository-wide search (`rg` via `bash`) to find every consumer, import, or reference outside the diff.
...
A changed exported symbol with zero recorded search commands in the trace is a skill violation. You are not allowed to assert "this is only used here" without showing the search.
