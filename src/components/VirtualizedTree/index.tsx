import React, { useRef, useCallback, useLayoutEffect, forwardRef, useImperativeHandle } from 'react'
import { Virtuoso, VirtuosoHandle, ListRange } from 'react-virtuoso'
import styled from 'styled-components'

export interface VirtualizedTreeItem {
  id: string
  [key: string]: any
}
...
interface VirtualizedTreeProps<T extends VirtualizedTreeItem> {
  items: T[]
...
function VirtualizedTreeComponent<T extends VirtualizedTreeItem>(
  ...
  const findItemIndex = useCallback((id: string) => {
    return items.findIndex(item => item.id === id)
  }, [items])

  const navigateInTree = useCallback((options: NavigationOptions) => {
    ...
    switch (to) {
      ...
      case "next":
        ...
        const nextIndex = Math.min(items.length - 1, findItemIndex(elementId) + 1)
        virtuosoRef.current.scrollIntoView({
          index: nextIndex,
      ...
      case "pageUp":
        ...
        const upIndex = focusedIndex !== null && focusedIndex !== upStart
          ? upStart
      ...
      case "pageDown":
        ...
        const downIndex = focusedIndex !== null && focusedIndex !== downEnd
          ? downEnd
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    if (focusedIndex === null && items.length > 0) {
      if (['ArrowDown', 'ArrowUp', 'Tab', 'Enter', 'Home', 'End', 'PageUp', 'PageDown'].includes(e.key)) {
        e.preventDefault()
        setFocusedIndex(0)
        return
      }
...
export const VirtualizedTree = forwardRef(VirtualizedTreeComponent) as <T extends VirtualizedTreeItem>(
