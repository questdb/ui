...
export const SearchPanel = React.forwardRef<SearchPanelRef, SearchPanelProps>(
  ({ open }, ref) => {
    ...
    const pendingUpdatesRef = useRef<
      Record<number, Record<number, PendingUpdate>>
    >({})
    const currentSearchResultRef = useRef<SearchResult>(searchResult)
    const currentStaleBuffersRef = useRef<number[]>([])
    const bufferSearchTimesRef = useRef<
      Record<number, { duration: number; query: string }>
    >({})
    const singleSearchAbortControllersRef = useRef<
    ...
    const performSearch = useCallback(async () => {
      ...
      if (
        createErrorTermsString(searchQuery, searchOptions) ===
        lastErrorTermsRef.current
      ) {
        return
      }
      if (!searchQuery.trim()) {
      ...
      try {
        ...
        const generator = SearchService.searchInBuffers(
          allBuffers,
          searchQuery,
          searchOptions,
        ...
        if (currentVersion === searchVersionRef.current) {
          ...
          if (e instanceof SearchTimeoutError) {
            ...
            lastErrorTermsRef.current = createErrorTermsString(
              searchQuery,
              searchOptions,
            )
          } else {
        ...
        if (currentVersion === searchVersionRef.current) {
          ...
          const pendingForCurrentVersion =
            pendingUpdatesRef.current[currentVersion]
          if (
            pendingForCurrentVersion &&
            Object.keys(pendingForCurrentVersion).length > 0
          ) {
            setTimeout(async () => {
              ...
              const {
                result,
                staleBuffers: newStaleBuffers,
                buffersToRefresh,
              } = await applyPatchesByVersion(
                currentVersion,
                currentResult,
                currentStaleBuffers,
              )
    ...
    const refreshBuffers = async (bufferIds: number[]) => {
      for (const bufferId of bufferIds) {
        const existingController =
          singleSearchAbortControllersRef.current.get(bufferId)
        ...
        try {
          const newMatches = await performSingleSearch(
            bufferId,
            abortController.signal,
            10000,
          )
          ...
          setSearchResult((prev) => {
            const firstMatchIndex = prev.matches.findIndex(
              (m) => m.bufferId === bufferId,
            )
    ...
    const applyPatchesByVersion = async (
      version: number,
      ...
      for (const [bufferId, update] of Object.entries(currentVersionUpdates)) {
        const {
          delete: deleteUpdate,
          metaUpdate,
          contentUpdate: contentUpdateInitial,
        } = update
        ...
        if (deleteUpdate) {
          updatedMatches = updatedMatches.filter(
            (match) => match.bufferId !== bufferIdNum,
          )
          updatedStaleBuffers = updatedStaleBuffers.filter(
            (id) => id !== bufferIdNum,
          )
        }
        ...
        if (contentUpdate) {
          ...
          const isFastBuffer =
            bufferSearchTimesRef.current[bufferIdNum]?.query === searchQuery &&
    ...
    useEffect(() => {
      const handleBuffersUpdated = (payload?: BufferUpdatePayload) => {
        ...
        if (type !== "deleteAll") {
          pendingUpdatesRef.current[searchVersionRef.current] =
            pendingUpdatesRef.current[searchVersionRef.current] || {}
        }
        const currentVersionUpdate =
          pendingUpdatesRef.current[searchVersionRef.current]
          ...
          case "update": {
            ...
            if (currentVersionUpdate) {
              currentVersionUpdate[bufferId] = {
                ...
                metaUpdate:
                  payload?.metaUpdate ||
                  currentVersionUpdate[bufferId]?.metaUpdate,
                contentUpdate:
                  payload?.contentUpdate ||
                  currentVersionUpdate[bufferId]?.contentUpdate,
              }
            }
            break
          }
          case "archive": {
            ...
            if (currentVersionUpdate) {
              currentVersionUpdate[archiveBufferId] = {
                ...
                metaUpdate: true,
                contentUpdate:
                  currentVersionUpdate[archiveBufferId]?.contentUpdate,
              }
            }
            break
          }
