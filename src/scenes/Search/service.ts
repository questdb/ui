  ...
  static async searchInSingleBuffer(
    ...
    try {
      ...
      for (const titleMatch of titleMatches) {
        ...
        matches.push(
          this.createTitleMatch(buffer, titleMatch.start, titleMatch.end),
        )
      }
  ...
  private static async searchInBuffer(
    ...
    try {
      ...
      if (e instanceof SearchTimeoutError) {
        ...
        const partialSearchMatches =
          e.partialMatches?.map((match) => ({
            bufferId: buffer.id!,
            bufferLabel: buffer.label,
            range: {
              startLineNumber: match.lineNumber,
