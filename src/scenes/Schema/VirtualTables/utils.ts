...
export const createSymbolDetailsNodes = (
  details: SymbolColumnDetails,
  parentId: string,
): TreeNode[] => {
  return [
    {
...
export const createSymbolDetailsPlaceholderNodes = (
  parentId: string,
): TreeNode[] => {
  return [
    {
...
const createColumnNodes = (
  ...
  return columns.map((column) => {
    ...
    const columnNode: TreeNode = {
      ...
      children:
        isExpanded && column.data_type === "SYMBOL"
          ? createSymbolDetailsPlaceholderNodes(columnId)
          : [],
    }
...
export const getNodeFromSchemaTree = (
  schemaTree: SchemaTree,
  id: string,
): { node: TreeNode; parent: SchemaTree | TreeNode } | undefined => {
...
export const updateAndGetSchemaTree = (
  schemaTree: SchemaTree,
  id: string,
  ...
  if (node) {
    ...
    if (parent.id) {
      parent.children = (parent.children as TreeNode[]).map(
        (child: TreeNode) => (child.id === id ? updatedNode : child),
      )
    } else {
