...
describe("compactSQL", () => {
  it("should remove consecutive spaces, new lines and leading/trailing spaces", () => {
    const sqlQuery = `
            ...
            ORDER BY name ASC  
        `
    expect(compactSQL(sqlQuery)).toBe(
      "SELECT id,name FROM users WHERE(age>21)AND(salary*2>5000)ORDER BY name ASC",
    )
  })
  ...
  it("should handle nested queries correctly", () => {
    const sqlQuery = `
        ...
        `
    expect(compactSQL(sqlQuery)).toBe(
      "SELECT id FROM(SELECT id FROM users WHERE age>21)AS subquery;",
    )
  })
