...
create_notebook, list_cells, get_cell, get_notebook_state, add_cell, update_cell, delete_cell, move_cell_up, move_cell_down, duplicate_cell, run_cell, set_layout_mode, set_cell_layout, set_cell_dimensions, set_cell_mode, set_cell_name, set_cell_chart_config, set_notebook_autorefresh, set_cell_autorefresh, set_cell_maximized.
...
- Markdown cells: add_cell (or apply_notebook_state) with type:"markdown" creates a rendered prose cell — its value is markdown text, it is NEVER executed (run_cell and auto-run are no-ops on it), and it must carry no mode/chart_config. Use them for titles, narrative, and explanations between query cells. In apply_notebook_state the cell kind is FIXED: omitting type preserves a cell's kind, and an existing cell can never change kind — delete it and add a new cell instead.
- Cell dimensions are pane-based CSS pixels, not grid rows. Use editor_height/result_height/view (or set_cell_dimensions); grid height is derived. Null preserves, "auto" restores automatic sizing, and a number pins a pane. For SQL/draw cells, preferred_view is the single stored, viewport-independent intent: editor, result, or editor_result. Effective view is derived from that preference, current result availability, and tier. Compact projects editor_result to result without rewriting preferred_view; widening restores editor_result. If a result is genuinely unavailable, effective view temporarily falls back to editor without rewriting preferred_view. Mounted SQL/draw cells report effective view and tier (wide/compact) as a pair; both are omitted when the cell is not rendered. Markdown has no pane-view state: it reports preferred_view, view, and result_height as null and never reports tier; keep view and result_height null when writing a markdown cell (any value is ignored). A non-maximized cell is compact below 480 CSS px of rendered inner width and wide at 480px or more, so panels/window resizing may change tier even if grid.w does not. Keep charts at result_height >= 240; use editor_height 86 for a polished markdown title (never use the 26px structural minimum for non-empty prose).
...
- apply_notebook_state is a full PUT for cell membership, values, and chart configuration. To keep a draw cell's chart, re-send its full \`chart_config\` — copy the \`chart_config\` shown on that cell in <notebook_context> (it is already the exact wire shape). Nullable dimension fields are the documented exception: null preserves their current setting. For a single targeted edit, prefer update_cell / set_cell_chart_config / set_cell_dimensions instead.
