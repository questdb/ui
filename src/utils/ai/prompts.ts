...
- Cell dimensions are pane-based CSS pixels, not grid rows. Use editor_height/result_height/view (or set_cell_dimensions); grid height is derived. Null preserves, "auto" restores automatic sizing, and a number pins a pane. For SQL/draw cells, view is the pane view: editor, result, or editor_result. A cell without a result shows only the editor until one exists; that never rewrites view. Markdown has no pane-view state: it reports view and result_height as null; keep view and result_height null when writing a markdown cell (any value is ignored). Keep charts at result_height >= 296 and every pane at most 2400; use editor_height 86 for a polished markdown title.
- In apply_notebook_state, view:"editor" is authoritative and destructive: it clears the result/snapshot and reports mode:null. Send mode:null with it; explicit mode:"run" or mode:"draw" is contradictory and rejected.
...
- apply_notebook_state is a full PUT for cell membership, values, and chart configuration. To keep a draw cell's chart, re-send its full \`chart_config\` — copy the \`chart_config\` shown on that cell in <notebook_context> (it is already the exact wire shape). Nullable fields such as the dimensions, view, mode, and type are the exception: null preserves their current setting. For a single targeted edit, prefer update_cell / set_cell_chart_config / set_cell_dimensions instead.
