...
- Cell dimensions are pane-based CSS pixels, not grid rows. Use editor_height/result_height/view (or set_cell_dimensions); grid height is derived. Null preserves, "auto" restores automatic sizing, and a number pins a pane. view is editor, result, or editor_result. Wide cells support all three; compact cells support editor/result and project editor_result to result, restoring editor_result if they become wide again. preferred_view is always the stored intent. Mounted cells additionally report effective view and tier (wide/compact) as a pair; both are omitted when the cell is not rendered. A non-maximized cell is compact below 480 CSS px of rendered inner width and wide at 480px or more, so panels/window resizing may change tier even if grid.w does not. Keep charts at result_height >= 240; use editor_height 86 for a polished markdown title (never use the 26px structural minimum for non-empty prose).
